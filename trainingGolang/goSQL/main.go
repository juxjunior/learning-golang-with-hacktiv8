...
Returning *
`
	err = db.QueryRow(sqlStatement, "Hapus Aku", "naga@pegadaian.co.id", 99, "Penjahat").
...
func UpdateEmployee() {
	...
	res, err := db.Exec(sqlStatement, 4, "Pertiwi Asri", "tiwi@gmail.com", "PKBL", 24)
	if err != nil {
		panic(err)
	}
...
func DeleteEmployee() {
	...
	WHERE id = $1;
	`
	res, err := db.Exec(sqlStatement, 5)
	if err != nil {
		panic(err)
	}
	count, err := res.RowsAffected()
